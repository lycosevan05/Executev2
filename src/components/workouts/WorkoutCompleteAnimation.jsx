...
export default function WorkoutCompleteAnimation({ onDismiss, workoutName }) {
  useEffect(() => {
    confetti({
      particleCount: 120,
      spread: 80,
    ...
    >
      ...
      >
        ...
        <motion.p
          ...
          style={{ color: '#5d635d' }}
        >
          {workoutName ? `You crushed ${workoutName}.` : 'Every set done.'} Fuel up and let your body recover — that's where the gains happen.
        </motion.p>
        ...
        >
          {['💪 Strength', '🔥 Calories', '⚡ Performance', '😴 Recover'].map((tag, i) => (
            <motion.span
              key={tag}
